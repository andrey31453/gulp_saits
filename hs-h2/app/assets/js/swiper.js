	...
	const B = {
			...
			loop: {
				loopCreate: function () {
					...
					for (let e = n.length - 1; e >= 0; e -= 1)
						i.prepend(
							d(n[e].cloneNode(!0)).addClass(s.slideDuplicateClass)
						)
				},
				loopFix: function () {
					const e = this
			...
			grabCursor: {
				setGrabCursor: function (e) {
					...
					if (
						...
						return
					const s =
						'__container' === t.params.touchEventsTarget
							? t.el
							: t.wrapperEl
					;(s.style.cursor = 'move'),
						(s.style.cursor = e
				...
				unsetGrabCursor: function () {
					...
					e.support.touch ||
						...
						e.params.cssMode ||
						(e[
							'__container' === e.params.touchEventsTarget
								? 'el'
								: 'wrapperEl'
						].style.cursor = '')
				},
			...
			events: {
				attachEvents: function () {
					...
					;(e.onTouchStart = S.bind(e)),
						...
						i.touch &&
							!L &&
							(t.addEventListener('touchstart', I), (L = !0)),
						A(e, 'on')
				},
	...
	const J = [
		function ({ swiper: e, extendParams: t, on: s }) {
			...
			function i(t) {
				...
				;(m = e.rtlTranslate
					...
					: 'top'),
					r
						? ((f = Math.floor(s / 2) + i + l),
						  (g = Math.floor(s / 2) + i + n))
						: ((f = s + (i - 1) + l), (g = i + n))
		...
		function ({ swiper: e, extendParams: t, on: s, emit: a }) {
			...
			function g(t) {
				...
				let o = e.$el
				if (
					('__container' !== e.params.mousewheel.eventsTarget &&
					...
					if (!i) {
						...
						if (
							...
							if (
								(c.push(t),
								s &&
									(t.delta > s.delta || t.direction !== s.direction))
							)
								c.splice(0)
							else if (
								c.length >= 15 &&
				...
				if (
					...
					if (
						(s.dynamicBullets &&
							...
							(i = p - n),
							(l =
								i + (Math.min(a.length, s.dynamicMainBullets) - 1)),
							(u = (l + i) / 2)),
						a.removeClass(
							[
								'',
					...
					setTranslate: function (t, s) {
						...
						function o(t) {
							...
							'slide' === e.params.controller.by &&
								...
								(n && '__container' !== e.params.controller.by) ||
									((r =
										(t.maxTranslate() - t.minTranslate()) /
										(e.maxTranslate() - e.minTranslate())),
									(n =
										(s - e.minTranslate()) * r + t.minTranslate())),
		...
		function ({ swiper: e, extendParams: t, on: s }) {
			...
			function v() {
				...
				const u = e.params.loop
					? e.slides.filter(
							(t) =>
								!t.classList.contains(e.params.slideDuplicateClass)
					  ).length
					: e.slides.length
				let m, v
				e.slides.each((s, a) => {
						...
						if ('row' === p && o > 1) {
							const e = Math.floor(t / (o * c)),
								...
								i =
									0 === e
										? o
										: Math.min(Math.ceil((n - e * c * o) / c), o)
							;(m = Math.floor(a / i)),
								(h = a - m * i + e * o),
								(u = h + (m * s) / c),
